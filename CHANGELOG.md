...
ownership feature introduced.

### Changed
- **The kit no longer claims independence on a publisher's behalf.** Every page
  carried *"Independent and unaffiliated with any government body, civic
  association, or the businesses mentioned"* — asserted automatically, with
  nothing in `/setup` ever asking. It is a stronger case than any claim fixed in
  v0.15.0: the others were statements about the *system*, which code can be
  inspected for; this is a statement about the *publisher*, which the software
  cannot know. A city employee, school board member, association officer or
  main-street shop owner generated a false statement of fact about themselves,
  in the footer of every page, and would have had to notice it to know.

  Now `affiliationNote` in `site.config.json`, **empty by default** — the page
  says nothing about affiliation rather than asserting something nobody
  verified. `/setup` asks directly; a publisher with ties writes a one-line
  disclosure, one without may write the sentence themselves and mean it.

### Fixed
- **The footer's half of the missing-inbox defect.** v0.14.0 fixed this in the
  About page; the footer still deleted its entire "spotted a mistake?" block when
  `contactEmail` was empty. Net effect on a real instance: **every page** said
  *"generated by an AI system… it may contain errors"* and then offered no way to
  report one. The footer now says so plainly instead of hiding it.

### Added
- **Drift detection for an ejected About page.** Ownership freezes the prose —
  that is the point of ejecting, and its cost: a field instance ejected at
  v0.14.0 and then sat on absolute privacy claims that v0.15.0 had already
  corrected, with everything green and nothing anywhere saying so. `--eject-about`
  now stamps the file with the About-prose revision, and doctor emits a **WARNING**
  (never fatal, never touching the file) when the kit's revision is newer, naming
  what to do. Generalises to any eject-and-own feature: the fix for staleness is
  not to un-own the artifact, it is to make the drift legible.

*Verified: both independence claims absent from the rendered footer and About
page; a non-empty `affiliationNote` renders in both; the footer admits the missing
inbox; the drift warning is silent at the current revision, fires on an older
stamp, and leaves doctor's exit code at 0 because it is advisory. Reasoned:
nothing.*
