...
GitHub firing one cron at the right hour.

0.26.0 removed a defect that would have cost every copy its first edition. This release is
about what happens when something *else* fails — because for a publisher who is not reading
the Actions tab on a Saturday, "the pipeline noticed and opened a ticket" and "nothing
happened" are the same event.

### Added

- **`bin/recovery-plan.mjs`** — the only thing in the kit that decides, unsupervised, either
  to publish or to spend money. Given the run list for a week it returns exactly one of
  `recover`, `retry` or `give-up`, and it is a pure function of that list so every branch can
  be driven offline. `doctor` drives all six.
- **`publication-check.yml` repairs instead of only reporting.** If a failed run left a
  usable artifact, that edition is downloaded, **put through every gate again**, rebuilt and
  pushed. If nothing is recoverable it dispatches **one** retry. If it has already retried it
  stops and says so. It opens an issue either way — including when it succeeded, because a
  repair nobody audits is a repair nobody can question.
- **`bin/cron-decision.mjs`** and **three crons in `weekly.yml`**, generated by
  `sync-crons.mjs`. GitHub cron is UTC and does not follow daylight saving, so one Friday
  entry publishes at 16:00 local in summer and 15:00 in winter — this kit used to say "shift
  the hour or accept the drift", which is a defect with an explanation attached. Both UTC
  hours that land on your local publish hour are now scheduled and the preflight keeps
  whichever is correct that week. In a timezone that does not change its clocks the two
  collapse to one.
- **A backstop cron**, hours after the publish, which declines in seconds unless the week is
  genuinely unpublished. GitHub silently drops scheduled runs during platform degradation and
  never retries them; nothing else in the system can see a week that never started.
- **`publishLocalHour` in `site.config.json`**, written automatically by `sync-crons.mjs` the
  first time it runs. Pinning it is what makes the publish hour survive the clocks changing —
  deriving it fresh each time would read whatever offset is in force today, and the guard
  meant to stop the hour drifting would drift with it.
- **A warning before GitHub disables your schedule.** Scheduled workflows stop after 60 days
  of repository inactivity, and when they do the weekly issue, the publication check and the
  smoke test stop together — the watchdog is switched off along with what it watches. The
  daily smoke test warns from day 45.

### Changed

- **The truncated-draft floor is measured, not remembered.** It was a constant with a comment
  telling the publisher to "raise it once you know your own typical size" — a calibration
  step nobody performs, which goes stale the first time the format changes. The reference
  implementation shipped the same idea and it failed exactly that way: its floor ended up
  *above* two real published editions. It is now the median of your last six, with an
  absolute floor for a publication that has none yet. That mattered more once
  `publication-check.yml` gained the ability to act on it — **adding an actuator to a
  detector raises the price of a miscalibrated threshold.**

### Verified

- `doctor` 29 checks, up from 27.
- The cron decision driven across four timezone shapes × both halves of the year: US Eastern,
  US Pacific (where the second slot crosses into the next UTC **day**, the rollover this kit
  already has a scar about), Arizona (no daylight saving, so the two collapse to one) and
  Sydney (southern hemisphere, offsets the other way round). **Exactly one cron publishes per
  publication day in every case.** Backstop silent when the week published, firing when it did
  not, and dated to the publish day rather than the day it runs.
- The recovery planner driven through all six branches, and mutation-tested on the two that
  spend money: disabling the recover branch makes it plan `retry` on a recoverable edition;
  removing the retry budget makes it plan `retry` on a week already retried.
- The size floor driven against a synthetic corpus: a normal edition passes, a short one
  passes, a truncated one fails — and, for a brand-new kit with nothing to calibrate against,
  a first issue passes on the absolute floor while a truncated first issue still fails.
- Three mutations on the cron decision. The third — dating the backstop to the day it runs
  rather than the publish day — was **not** caught by the first version of the test, because a
  six-hour backstop is still the same local day. A case where they differ was added, and then
  it was caught. A test whose cases cannot distinguish the implementations is not a test.
